...
def generate(mu, log_var, log_pi, n_mixtures, device):
    _, max_indices = torch.max(log_pi, dim=-1) # (N, T, n_bins) range 0~4

    # y_phn to 1-hot encoding (N, T) => (N, T, V)
    # y_onehot = y_phn.numpy()
    # y_onehot = y_onehot.flatten().astype(int)
    # y_onehot = np.eye(Net1TimitData.phone_vocab_size)[y_onehot]
    # y_onehot = torch.from_numpy(y_onehot).type(torch.LongTensor)
    # flatten logits and y_phn

    # max_indices to 1-hot encoding
    argmax = to_one_hot(max_indices)
    dtype = torch.FloatTensor
    if device > 0:
        dtype = torch.cuda.FloatTensor

    # mask mixtures otehr than the argmax
    mu = mu.data[0].type(torch.FloatTensor)
    mask = torch.mul(mu,argmax)
    pred_spec = torch.sum(mask, dim=-1)
    pred_spec = torch.autograd.Variable(pred_spec).type(dtype)
    return pred_spec

def pdf_parameter_extraction(out, n_mixtures, batch_size):
    n_bins = hp.default.n_fft//2+1
    n_units = n_bins * n_mixtures
    mu = F.sigmoid(out[..., :n_units])  # (N, T, 1285)
    mu = mu.contiguous().view(batch_size, -1, n_bins, n_mixtures)  # (N, T, 257, 5)

    log_var = torch.clamp(out[..., n_units: 2 * n_units], min=-7.0)
    log_var = log_var.view(batch_size, -1, n_bins, n_mixtures)  # (N, T, 257, 5)

    log_pi = out[..., 2 * n_units: 3 * n_units]
    #log_pi = log_pi.transpose(2,1)
    #instance_norm = torch.nn.InstanceNorm1d(log_pi.shape[1], eps=1e-8, momentum=0.001)
    #if hp.train2.device > 0:
    #    log_pi.type(torch.cuda.FloatTensor)
    #log_pi = F.batch_norm(log_pi, eps=1e-8, momentum=0.001)
    #log_pi = instance_norm(log_pi)
    #log_pi = log_pi.transpose(2,1)
    log_pi = log_pi.contiguous().view(batch_size, -1, n_bins, n_mixtures)
